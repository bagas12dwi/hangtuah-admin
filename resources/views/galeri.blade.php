...
@section('konten')
    <div class="row justify-content-between">
        ...
        <div class="col">
            <button type="button" class="btn btn-primary float-end" data-bs-toggle="modal" data-bs-target="#tambahGaleri">
                Tambahkan {{ $title }}
            </button>
        </div>
    </div>
                        ...
                        <tbody>
                            @foreach ($data as $data)
                                <tr>
                                    <td>
                                        <div class="my-auto">
                                            <img src="{{ asset('storage/galeri/' . $data->imgPath) }}" width="100px"
                                                alt="" />
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td class="align-middle">
                                        <button data-bs-toggle="modal" data-bs-target="#editGaleri{{ $data->id }}"
                                            class="edit btn font-weight-bold text-xs" data-original-title="Edit user"
                                            id="edit">
                                            Edit
                                        </button>
                                        <form action="/galeri/{{ $data->id }}" method="post" class="d-inline">
                                            @method('delete')
                                            @csrf
                                ...
                                <div class="modal fade" id="editGaleri{{ $data->id }}" tabindex="-1" role="dialog"
...
@include('components.modal-galeri')
